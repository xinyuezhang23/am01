...
author: "Study group 4 and members' names go here"
date: "`r Sys.Date()`"
output:
  html_document:
    theme: flatly
...
#cache=true download one time, don't need to download every time
...
#eval=FALSE change to TRUE then you can knit
...
```
The higher HIV prevalence, the lower life expectancy. The have a negative correlation.
...
view(mortality_improvement)
```
In mortality_improvement data frame, we can see mortality improvment over the past years is order by region and descending values. 
Take East Asia & Pacific region as an example. We can find that Korea,Rep ,Singapore ,Japan ,Thiland ,China have seen the greatest improvement. And Micronesia, Fed. Sts. ,Korea, Dem. People's Rep.,Palau, Nauru,Tuvalu have seen the least improvement. There is no deterioration in all regions.
...
```
Yes, a negative correlation. The higher fertility rate, the lower primary school enrollment rate.
...
```{r, rentals_distribution}
fav_stats(bike$bikes_hired)
...
```{r}
bikes_with_avg <- left_join(bike, monthly_avg, on='month')

bikes_with_avg <- bikes_with_avg %>% 
  filter(year %in% c(2016,2017,2018,2019,2020, 2021)) %>% 
  mutate(excess_rentals = bikes_hired - mean_bikes)

```
```{r}
bikes_with_avg %>% 
  ggplot(aes(x = factor(month), y=mean_bikes))+
