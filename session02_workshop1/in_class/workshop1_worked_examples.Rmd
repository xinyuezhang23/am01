...
ggplot(bike_weather, aes(x = month, y = temp, fill=month)) +
  geom_boxplot()
```
...
- Colour the bars by `genre`
```{r}
movies %>% 
  slice_max(order_by = gross, n=20) %>% 
  ggplot(aes(x = gross, y = fct_reorder(title, gross),fill= genre)) +
  geom_col() +
  theme_bw()+
  labs(
    title = "",
...
Inside aes, fill the variables; in geom_col, fill bars
```{r}
movies %>% 
  slice_max(order_by = gross, n=20) %>% 
  ggplot(aes(x = gross, y = fct_reorder(title, gross))) +
...
```{r}
movies %>% 
  slice_max(order_by = return_on_budget, n=20) %>% 
  ggplot(aes(x = return_on_budget, y =fct_reorder(title,return_on_budget))) +
  geom_col()
  theme_bw()+
  labs(
    title = "",
...
gapminder %>%
  ...
  mutate(
    country = fct_reorder(country, lifeExp, median) # reorder country by median(lifeExp)
  ) %>%
  ggplot(aes(year, country, fill = lifeExp)) + 
  geom_tile() + # It's the same if we use geom_col()here
  scale_fill_viridis_c(option = "A") #option here represents different fill color
```
